...
- **多协议互转支撑**：
  ...
  - **Responses API ➔ Chat Completions**：支持新版 OpenAI Responses 规范，将输入节点队列（messages, reasoning, function_calls）映射到普通的 chat/completions 模型上（如 DeepSeek R1），并完整模拟流式与非流式事件输出，使项目无缝适配新版 Codex。
- **可视化管理控制台**：
...
- **`npm run test`**：运行网关协议转换单元测试（验证角色交替、合并、工具调用等翻译逻辑）。
- **`npm run lint`**：运行 ESLint 静态代码风格与安全校验。
- **`npm run dev:frontend`**：在本地拉起控制台前端开发服务器（Vite 热更新调试）。
- **`npm run build:frontend`**：编译 React 前端静态资源输出到 `frontend/dist/`，编译产物将自动被网关后端托管。
